# Lingaro Azure Databricks Python 3.12 DevContainer

A comprehensive Azure Databricks development environment running Python 3.12 on Ubuntu with integrated AI tooling and cloud development capabilities. Built from the GitHub Container Registry image `ghcr.io/lingaro/lingaro-azure-databricks-py312-ubuntu24:latest`.
...
1. **Get the setup script** and put it in a desired empty folder:
   ...
   - Try `curl -fsSLo get-devcontainer.sh https://raw.githubusercontent.com/Lingaro/devcontainers-templates/main/lingaro-azure-databricks-py312-ubuntu24/get-devcontainer.sh`
   - or `wget -O get-devcontainer.sh https://raw.githubusercontent.com/Lingaro/devcontainers-templates/main/lingaro-azure-databricks-py312-ubuntu24/get-devcontainer.sh`
...
```bash
# Install packages
uv pip install requests pandas numpy databricks-sdk
...
3. **Environment Variables**: Keep sensitive data in `.env` file, never commit it to version control
4. **Package Management**: Use `uv` for faster package installation
5. **Version Control**: Use `.gitignore` to exclude `.env` and virtual environments
6. **Resource Cleanup**: Stop containers when not in use to save resources
7. **Least Privilege**: Configure Azure service principal with minimal required permissions
...
This template provides a robust foundation for Azure Databricks and AI-powered Python development with modern tooling and best practices built-in.
